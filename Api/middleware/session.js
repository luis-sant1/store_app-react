...
const authMidd = async (req, res, next) => {
    try {
        if(!req.headers.authorization){                                                         // Buscamos en lso headers el Bearer token
        ...
        req.user = user                                                                         // Inyectamos la propiedad user al req que es igual al user retornado por la DB. 
        next()                                                                                  // Ya en cualquier otro controlador podemos saber qué usuario está accediendo a ese endpoint (user = req.user)
    } catch (error) {
        console.log(error)
